#!/usr/bin/env python3
"""
api_google.py — Valida permissões de uma Google API Key.

Uso:
    python3 api_google.py <API_KEY>
    python3 api_google.py <API_KEY> --json          # saída em JSON
    python3 api_google.py <API_KEY> -o results.json # salva em arquivo
    python3 api_google.py --file keys.txt           # testa múltiplas chaves
"""
...
def build_endpoints(key: str) -> dict[str, str]:
    return {
        ...
        "Places Details":       f"https://maps.googleapis.com/maps/api/place/details/json?place_id=ChIJN1t_tDeuEmsRUsoyG83frY4&fields=name&key={key}",
        ...
        "Cloud Vision":         f"https://vision.googleapis.com/v1/images:annotate?key={key}",
        "Cloud Storage":        f"https://storage.googleapis.com/storage/v1/b?project=test&key={key}",
        "Geolocation":          f"https://www.googleapis.com/geolocation/v1/geolocate?key={key}",
    }
...
def classify(r: requests.Response) -> tuple[str, str]:
    ...
    if "API key not valid" in text:
        note = "chave rejeitada pela API" if r.status_code == 400 else ""
        return STATUS_INVALID, note
...
def test_key(api_key: str, workers: int = 8, timeout: int = 10) -> list[CheckResult]:
    ...
    # Ordena: vulneráveis → sem permissão → negado → erro → inválida → falha
    order = {
        STATUS_OK:      0,
        STATUS_NO_PERM: 1,
        STATUS_DENIED:  2,
...
def print_results(api_key: str, results: list[CheckResult]) -> None:
    ...
    if vuln:
        print(f"\n{BOLD}{COLORS[STATUS_OK]}⚠ ATENÇÃO — {len(vuln)} endpoint(s) acessível(is):{RESET}")
        for r in vuln:
            print(f"  • {r.name}")
    else:
        print(f"\n{COLORS[STATUS_DENIED]}✓ Nenhum endpoint acessível com essa chave.{RESET}")
    print()
...
def save_output(api_key: str, results: list[CheckResult], filepath: str) -> None:
    """Salva resultado em arquivo JSON, acumulando se já existir."""
    import os
    existing: list[dict] = []
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output",  "-o",  default=None,        help="Salvar resultado em JSON (ex: -o results.json)")
...
def main() -> None:
    ...
    if args.file:
        ...
        print("Uso: python3 api_google.py <API_KEY> [--json] [--file keys.txt]")
        sys.exit(1)
    ...
    for key in keys:
        ...
        if args.output:
            save_output(key, results, args.output)
        if any(r.status == STATUS_OK for r in results):
            any_vuln = True
